...
public class PaymentRideConsumer {
    private final PaymentService paymentService;

    @RabbitListener(queues = "${ride-service.payment.queue}", ackMode = "MANUAL")
    public void handlePaymentRideMessage(FinishRideRequest message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long tag) throws IOException {
        try {
            log.info(RECEIVED_MESSAGE, message);

            switch (message.getAction()) {
                case "COMPLETE":
                    paymentService.handleCompletePayment(message);
                    break;
                default:
